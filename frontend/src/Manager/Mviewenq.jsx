...
function Mviewenq() {
    ...
    const getenq = async()=>{
        ...
        if(res.data.msg=="sucess"){
            ...
            var enquries = res.data.enq.filter((e)=>{
                return e.center == res2.data.user.center
            })
            console.log(enquries);
            setEnq(enquries);

            // var enquires = res.data.enq.filter((a)=>{
            //     return a.assignto?a.assignto._id==localStorage.getItem('cons'):true
            // })
...

        {
            filterfollow.map((f)=>(
                <div className='row my-3'>
                    <div className="col-md-4">
                        <p className='bg-warning text-center rounded-3 p-1'>{f.status}</p>
                    </div>
                    <div className="col-md-8">
                        {f.createdAt}
                    </div>
                    <div className="row">
                        <div className="col-md-12">
                            ...
                            <div className="row">
                                <div className="col-md-12">
                                  <p className='bg-light p-2 rounded-2'>by :- {f.uid.name} | {f.nextdate}</p>  
                                </div>
                            </div>
                            <hr />
                        </div>
